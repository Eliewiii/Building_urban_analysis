...
if _bipv_parameters is not None:
    if isinstance(_bipv_parameters, dict) is False or "roof_pv_tech_id" not in _bipv_parameters.keys() or "facade_pv_tech_id" not in _bipv_parameters.keys() or "minimum_panel_eroi" not in _bipv_parameters.keys() or "efficiency_computation_method"not in _bipv_parameters.keys():
        raise ValueError("The BIPV parameters are not valid, use the BIPV Parameters component as input")
    else :
        roof_pv_tech_id = _bipv_parameters["roof_pv_tech_id"]
        facade_pv_tech_id = _bipv_parameters["facade_pv_tech_id"]
        minimum_panel_eroi = _bipv_parameters["minimum_panel_eroi"]
        efficiency_computation_method = _bipv_parameters["efficiency_computation_method"]
else:
...
if _replacement_scenario_parameters is not None:
    if isinstance(_replacement_scenario_parameters, dict) is False or "replacement_scenario_id" not in _replacement_scenario_parameters.keys() or "replacement_frequency" not in _replacement_scenario_parameters.keys() or "minimal_panel_age" not in _replacement_scenario_parameters.keys():
        raise ValueError("The replacement scenario parameters are not valid, use the Replacement Scenario Parameters component as input")
    else :
        replacement_scenario_id = _replacement_scenario_parameters["replacement_scenario_id"]
        replacement_frequency = _replacement_scenario_parameters["replacement_frequency"]
        minimal_panel_age = _replacement_scenario_parameters["minimal_panel_age"]
else:
