...
energy_efficiency = {"efficient": {"rooftech": list_eff_rooftech, "envtech": list_eff_envtech_eff},
                     "aesthetic": {"rooftech": list_eff_rooftech,
                                   "envtech": list_eff_envtech_aesth}}  # PV conversion efficiency
...
def calc_wb(nb_of_years, lifetime, shape):
    ...
    nb_of_years : time in years until failure
    lifetime : scale parameter of the function, >0
    shape : impacts the failure rate, shape>1 indicates that the failure rate increases over time (aging process)"""
    list_wb = []
...
def calc_wb_cumulative(nb_of_years, lifetime, shape):
    """ calculate cumulative Weibull distribution values """
    list_wb_cumulative = []
    for i in range(0, nb_of_years):
        val = (1 - np.exp(-(i / lifetime) ** shape))
        list_wb_cumulative.append(val)

    return list_wb_cumulative


def initialize_dic_energy(nb_of_years, list_tech, list_b_type):
    """ Make a dictionary to put in energy production values
     nb_of_years : time in years until failure
     list_tech : global variable (roof or facades tech)
     list_b_type :
     """

    # initialize area dictionary
    dic_energy = {}
...
def calc_energy_arr(list_b_type, construction_scenario, dic_irradiation, datasheet, rows, columns, list_scenario):
    ...
    for row in range(rows[0], rows[1]):
        for column in range(columns[0], columns[1]):
            if column == columns[0]:
                arr_irradiation[row - rows[0], column - columns[0]] = datasheet.cell(row,
                                                                                     column).value * area_factor_roof
            else:
                arr_irradiation[row - rows[0], column - columns[0]] = datasheet.cell(row,
                                                                                     column).value * area_factor_env
    ...
    for scenario in list_scenario:
        for b_type in list_b_type:
            ...
            for year in range(nb_of_years):
                if scenario == "roof_scenario":
                    dic_irradiation[year][b_type]["roof_scenario"]["irradiation"] = arr_irradiation[r, c] * factorlist[
                        year]
                elif scenario == "env_scenario":
                    dic_irradiation[year][b_type]["env_scenario"]["irradiation"] = arr_irradiation[r, c] * factorlist[
                        year]
                else:
                    print("error")
...
def calc_energy_production(dic_result_per_year, dic_area, construction_scenario, list_level, list_b_type, datasheet,
                           rows, columns, wb, level):
    ...
    arr_irradiation, dic_irradiation = calc_energy_arr(list_b_type, construction_scenario, dic_irradiation, datasheet,
                                                       rows, columns, list_scenario)
    for b_type in list(dic_result_per_year[0].keys()):
        ...
        for var in list_scenario:
            ...
            waste_arr, waste_modules_arr, list_inflow, list_inflow_modules = calc_cohorts(dic_result_per_year, b_type,
                                                                                          var, tech, wb, level,
                                                                                          nb_of_years)
            for year in range(nb_of_years):
                if var == "roof_scenario" or var == "env_scenario":
                    list_energy = []
                    module_check = []
                    left_weight_list, left_module_list, waste_list, waste_module_list = calc_left_modules(
                        dic_result_per_year, year, b_type, var, tech, wb, level, nb_of_years)
                    ...
                    for i in range(0, year + 1):
                        ...
                        if i == 0:
                            ...
                            if dic_result_per_year[i][b_type][var][tech]["modules"] > \
                                    dic_result_per_year[i - 1][b_type][var][tech]["modules"]:
                                new_modules_from_construction = dic_result_per_year[i][b_type][var][tech]["modules"] - \
                                                                dic_result_per_year[i - 1][b_type][var][tech]["modules"]

                            modules_from_i_in_year = new_modules_in_year_i + new_modules_from_construction - \
                                                     waste_module_list[i]
                            if modules_from_i_in_year < 0:
                                modules_from_i_in_year = 0

                        energy = (modules_from_i_in_year / modules_total_in_year_i * dic_irradiation[i][b_type][var][
                            "irradiation"]
                                  / pv_efficiency * energy_efficiency[level][tech][i] * pv_performance_ratio / 1000 * (
                                              1 - deg))
    ...
    for year in list(dic_energy.keys()):
        for b_type in list(dic_energy[year].keys()):
            for tech in list(dic_energy[year][b_type]["total_scenario"].keys()):
                ...
                # Total
                dic_energy[year][b_type]["total_scenario"][tech][
                    "energy_production"] = roof_energy_production + env_energy_production
...
def run_energy(dic_energy, dic_result_per_year, wb, level, construction):
    ...
    dic_energy, dic_irradiation = calc_energy_production(dic_result_per_year, dic_area, construction, list_level,
                                                         list_b_type, area_file, [4, 5], [7, 9], wb, level)
