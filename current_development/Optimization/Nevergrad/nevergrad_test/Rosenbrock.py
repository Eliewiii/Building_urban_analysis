...
def eval_func(zob,int_var_1=0,int_var_2=0,float_var_1=0,float_var_2=0,cat_var=0):
    ...
    objective_value = 100*(int_var_2-int_var_1**2)**2 + (1-int_var_1)**2 # int_var_2 + float_var_1 + float_var_2 + (1 if cat_var == 'A' else 0)
...
instrumentation = ng.p.Instrumentation(
    int_var_1=ng.p.Scalar(lower=-20, upper=20).set_integer_casting(),  # Integer variable 1 with bounds 0 to 5
    int_var_2=ng.p.Scalar(lower=-20, upper=20).set_integer_casting(),  # Integer variable 2 with bounds 5 to 10
    float_var_1=ng.p.Scalar(lower=0, upper=0.1),  # Float variable 1 with bounds 0 to 5
    float_var_2=ng.p.Scalar(lower=0, upper=0.1),  # Float variable 2 with bounds 5 to 10
    cat_var=ng.p.Choice([0])  # Categorical variable with 3 categories
)
...
optimizer = ng.optimizers.cGA(parametrization=instrumentation, budget=budget, num_workers=1)
...
best_int_var_1 = best_individual
best_int_var_2 = recommendation.kwargs['int_var_2']
