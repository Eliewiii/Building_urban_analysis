...
from building.solar_radiations_and_panel.utils_solar_radiation import run_hb_model_annual_irradiance_simulation,move_radiation_results
    ...
    def run_annual_solar_radiation(self, building_id, hb_model_obj, context_shading_hb_aperture_list,
        ...
        if self.roof_sensorgrid_dict is not None:
            if self.results_dict["Roof"]["annual_panel_irradiance_list"] is None or overwrite:
                ...
                path_temp_result_folder_roof = path_folder_run_radiation_temp_roof = os.path.join(path_folder_simulation,
                                                                                      name_temporary_files_folder,
                                                                                      str(building_id), "Roof",
                                                                                      "annual_irradiance", "results",
                                                                                      "total")
                move_radiation_results(path_temp_result_folder=path_temp_result_folder_roof, path_result_folder=path_result_folder, result_file_name=annual_irradiance_file_name,
        ...
        if self.facade_sensorgrid_dict is not None:
            if self.results_dict["Facade"]["annual_panel_irradiance_list"] is None or overwrite:
                ...
                hb_model_copy_facade.properties.radiance.add_sensor_grid(SensorGrid.from_dict(self.facade_sensorgrid_dict))
                ...
                path_temp_result_folder_facade = path_folder_run_radiation_temp_facade = os.path.join(path_folder_simulation,
                                                                                      name_temporary_files_folder,
                                                                                      str(building_id), "Facade",
                                                                                      "annual_irradiance", "results",
                                                                                      "total")
                move_radiation_results(path_temp_result_folder=path_temp_result_folder_facade, path_result_folder=path_result_folder, result_file_name=annual_irradiance_file_name,
