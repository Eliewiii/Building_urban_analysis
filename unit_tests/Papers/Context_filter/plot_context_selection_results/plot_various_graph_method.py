...
from unit_tests.Papers.Context_filter.plot_context_selection_results.plot_box_chart import plot_xlog_box_chart
from unit_tests.Papers.Context_filter.plot_context_selection_results.parse_result_dict import \
...
def plot_computation_time_compared_to_ref_vs_mvfc(path_to_result_dict: str, building_type="All", no_raytracing=False, base_width: float = 0.1,
                                           fig_height: float = 6.,
                                           fig_width: float = 9.):
    """
    Plot the error compared to the reference vs the MVFC. in ansolute value of the error
    ...
    for sim_building_value in parsed_dict.values():
        ...
        for mvfc, result_dict in sim_building_value[raytracing_key].items():
            ...
            first_pass_duration = sim_building_value[raytracing_key][mvfc]["context_selection"]["first_pass"][
                "duration"]
            second_pass_duration = sim_building_value[raytracing_key][mvfc]["context_selection"]["second_pass"][
                "duration"]
    ...
    plot_xlog_box_chart(data=data, x_positions=x_positions, x_label="MVFC",
                        y_label="Computation_time difference [%]", title=f"Computation_time_compared_to_reference",
                        base_width=base_width, fig_height=fig_height, fig_width=fig_width)


if __name__ == "__main__":
    path_to_result_dict = r"C:\Users\elie-medioni\OneDrive\OneDrive - Technion\Ministry of Energy Research\Papers\CheckContext\Simulations_Elie\results\Saved_results\results_context_filter_lca_lab_5.json"
