...
class ShadeConstructionManager:
    ...
    def from_hb_face_or_aperture_to_shade(self, hb_object):
        """ Transform a HB Face or an Aperture (window) to a Shade object
        :param hb_object: HB object, can be a HB Face or an Aperture (window)
        :return hb_shade-construction: ShadeConstruction
        """
        # Get the solar and visible reflectance of the HB object
        solar_reflectance, visible_reflectance = self.get_solar_and_visible_reflectance(hb_object)
        ...
        if isinstance(hb_object, Face):
            hb_object_geo = hb_object.geometry
            ...
            hb_object_geo = hb_object.geometry  # copy of the geometry
            hb_object_geo = hb_object_geo.move(hb_object_geo.normal, 0.01)
        else:
            raise ValueError("The object with the identifier {} is not a Honeybee Face or Aperture, it cannot be handled "
                             "by the context filter".format(hb_object.identifier))
        hb_shade = Shade(
            identifier=hb_object.identifer + "_shade",
            geometry=hb_object_geo,
            is_detached=True,
...
def get_solar_and_visible_reflectance(self, hb_object):
    """
    Get the solar and visible reflectance of the HB object
    :param hb_object: HB object, can be a HB Face or an Aperture (window)
    ...
    if isinstance(hb_object, Face):
        face_construction = hb_object.properties.energy.construction
    ...
    # If the object is a Honeybee Aperture
        ...
        window_construction = hb_object.properties.energy.construction
